...
def create_mozaic(image_width, image_height, num_bins_x, num_bins_y, min_date, max_date, date_list):
    ...
    max_date = date_to_seconds(max_date)
    min_date = date_to_seconds(min_date)
    ...
    for x, y, date in date_list:
        i = int(x * num_bins_x)
        j = int(y * num_bins_y)
        k = date_to_seconds(date)
        mozaic[i, j] = k
    ...
    valid_dates = mozaic_values[valid_indices]

    # Interpolate values for all grid points with a cubic spline
    interpolated_dates = griddata(
        (valid_x_indices, valid_y_indices),
        valid_dates,
        (x_indices, y_indices),
        method='cubic',
        fill_value=np.nan
    ...
    if(np.isnan(interpolated_dates).any()):

        # Valid indices contain float values given as input + spline interpolation
        valid_indices = ~np.isnan(interpolated_dates)
        valid_x_indices = x_indices[valid_indices]
        valid_y_indices = y_indices[valid_indices]
        valid_dates = interpolated_dates[valid_indices]
    ...
    # Clip the interpolated dates to the specified range
    interpolated_dates = np.clip(interpolated_dates, min_date, max_date)

    fig, ax = plt.subplots(figsize=(10, 8))
    c = ax.pcolormesh(x_bins, y_bins, interpolated_dates.reshape(num_bins_y, num_bins_x), shading='auto')
    ...
    for i, j in zip(x_indices, y_indices):
        value = seconds_to_date(interpolated_dates[i + j * num_bins_x])
...
# How to use it #
image_width = 10 # the number of pixels for the width of the output image
image_height = 10 # the number of pixels for the height of the output image
num_bins_x = 2 # the number of horizontal "boxes" in the output mozaic image
num_bins_y = 5  # the number of vertical "boxes" in the output mozaic image
